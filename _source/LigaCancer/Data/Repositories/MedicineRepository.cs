...
namespace RVCC.Data.Repositories
{
    public class MedicineRepository : IDataRepository<Medicine>
    {
        ...
        private static IQueryable<Medicine> GetOrdinationMedicine(IQueryable<Medicine> query, string sortColumn, string sortDirection)
        {
            return sortColumn switch
            {
                "Quantity" => sortDirection == "asc" ? query.OrderBy(x => x.PatientInformationMedicines.Count) : query.OrderByDescending(x => x.PatientInformationMedicines.Count),
                _ => sortDirection == "asc" ? query.OrderBy(x => x.Name) : query.OrderByDescending(x => x.Name)
            };
        }

        private static IQueryable<Medicine> GetFilteredMedicines(IQueryable<Medicine> query, MedicineSearchModel medicineSearch)
        {
