...
export default (function () {
    ...
    function initPage() {
        let serviceTypeTable = $("#serviceTypeTable").DataTable({
            dom:
            ...
            drawCallback: function () {
                $(".editServiceTypeButton").click(function () {
                    global.openModal($(this).attr("href"), $(this).data("title"), initEditForm);
                });

                $(".deleteServiceTypeButton").click(function () {
                    initDelete($(this).data("url"), $(this).data("id"), $(this).data("relation") === "True");
                });
            }
        });

        $("#serviceTypeTable").attr("style", "border-collapse: collapse !important");

        $("#searchForm").off("submit").submit(function (e) {
            e.preventDefault();
            serviceTypeTable.search("").draw("");
        });

        $("#addServiceTypeButton").click(function () { 
            global.openModal($(this).attr("href"), $(this).data("title"), initAddForm);
        });
    }

    function initAddForm() {
        $.validator.unobtrusive.parse("#addServiceTypeForm");

        $("#addServiceTypeForm").off("submit").submit(function (e) {
            e.preventDefault();
                ...
                $.post($(form).attr("action"), form.serialize())
                    .done(function (data, textStatus) {
                        if (!data && textStatus === "success") {
                            ...
                            $("#serviceTypeTable").DataTable().ajax.reload(null, false);
                            global.swalWithBootstrapButtons.fire("Sucesso", "Tipo de serviço registrado com sucesso.", "success");
                        } else {
                            $("#modalBody").html(data);
                            initAddForm();
                        }
    ...
    function initEditForm() {
        $.validator.unobtrusive.parse("#editServiceTypeForm");

        $("#editServiceTypeForm").off("submit").submit(function (e) {
            e.preventDefault();
                ...
                $.post($(form).attr("action"), form.serialize())
                    .done(function (data, textStatus) {
                        if (!data && textStatus === "success") {
                            ...
                            $(".modal-backdrop").remove();
                            $("#serviceTypeTable").DataTable().ajax.reload(null, false);
                            global.swalWithBootstrapButtons.fire("Sucesso", "Tipo de serviço atualizado com sucesso.", "success");
                        } else {
                            $("#modalBody").html(data);
                            initEditForm();
                        }
    ...
    function initDelete(url, id, relation) {
        ...
        if (relation) {
            message = "Este Tipo de serviço está atribuído a pacientes, deseja prosseguir mesmo assim?";
        }
        ...
        global.swalWithBootstrapButtons.fire({
            ...
            preConfirm: () => {
                $.post(url, { id: id, __RequestVerificationToken: $("input[name=__RequestVerificationToken").val()  })
                    .done(function () {
                        $("#serviceTypeTable").DataTable().ajax.reload(null, false);
                        global.swalWithBootstrapButtons.fire("Removido!", "O Tipo de serviço foi removido com sucesso.", "success");
                    }).fail(function () {
