...
export default (function () {
    ...
    function initPage() {
        let expenseTypeTable = $("#expenseTypeTable").DataTable({
            dom:
            ...
            columns: [
                ...
                { data: "expenseTypeFrequency", title: "Frequência", name: "ExpenseTypeFrequency" },
                { data: "quantity", title: "Pacientes", name: "Quantity" }
            ],
            drawCallback: function () {
                $(".editExpenseTypeButton").click(function () {
                    global.openModal($(this).attr("href"), $(this).data("title"), initEditForm);
                });

                $(".deleteExpenseTypeButton").click(function () {
                    initDelete($(this).data("url"), $(this).data("id"), $(this).data("relation") === "True");
                });
            }
        });

        $("#expenseTypeTable").attr("style", "border-collapse: collapse !important");

        $("#searchForm").off("submit").submit(function (e) {
            e.preventDefault();
            expenseTypeTable.search("").draw("");
        });
    ...
    function initAddForm() {
        $.validator.unobtrusive.parse("#addExpenseTypeForm");

        $("#addExpenseTypeForm").off("submit").submit(function (e) {
            e.preventDefault();
                ...
                $.post($(form).attr("action"), form.serialize())
                    .done(function (data, textStatus) {
                        if (!data && textStatus === "success") {
                            ...
                            $("#expenseTypeTable").DataTable().ajax.reload(null, false);
                            global.swalWithBootstrapButtons.fire("Sucesso", "Tipo de despesa registrado com sucesso.", "success");
                        } else {
                            $("#modalBody").html(data);
                            initAddForm();
                        }
    ...
    function initEditForm() {
        $.validator.unobtrusive.parse("#editExpenseTypeForm");

        $("#editExpenseTypeForm").off("submit").submit(function (e) {
            e.preventDefault();
                ...
                $.post($(form).attr("action"), form.serialize())
                    .done(function (data, textStatus) {
                        if (!data && textStatus === "success") {
                            ...
                            $(".modal-backdrop").remove();
                            $("#expenseTypeTable").DataTable().ajax.reload(null, false);
                            global.swalWithBootstrapButtons.fire("Sucesso", "Tipo de despesa atualizado com sucesso.", "success");
                        } else {
                            $("#modalBody").html(data);
                            initEditForm();
                        }
    ...
    function initDelete(url, id, relation) {
        ...
        if (relation) {
            message = "Este Tipo de despesa está atribuído a pacientes, deseja prosseguir mesmo assim?";
        }
        ...
        global.swalWithBootstrapButtons.fire({
            ...
            preConfirm: () => {
                $.post(url, { id: id, __RequestVerificationToken: $("input[name=__RequestVerificationToken").val()  })
                    .done(function () {
                        $("#expenseTypeTable").DataTable().ajax.reload(null, false);
                        global.swalWithBootstrapButtons.fire("Removido!", "O Tipo de despesa foi removido com sucesso.", "success");
                    }).fail(function () {
